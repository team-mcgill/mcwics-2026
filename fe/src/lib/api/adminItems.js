...
async function getWithWalletAuth({ publicKey, signMessage, path }) {
  const wallet = getWalletAddress(publicKey)
...
export async function fetchAdminItems() {
  return parseResponse(await fetch(`${API_BASE_URL}/api/admin/items`))
}

export async function fetchUserAdminInventory({ publicKey, signMessage }) {
  return getWithWalletAuth({
    publicKey,
    signMessage,
    path: '/api/admin/items/inventory',
  })
}

export async function buyAdminItem({ publicKey, signMessage, itemId, paymentSignature }) {
  return postWithWalletAuth({
    publicKey,
    signMessage,
    path: '/api/admin/items/buy',
    body: {
      itemId,
      paymentSignature,
    },
